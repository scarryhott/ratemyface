...
import { browserObservationProbe } from "./browserControl";
import { withGithubToolTimeout } from "./operatorGithubTimeout";
...
export interface OperatorToolContext { runId:number; signalId:number; admittedAuthority:Authority; }
export interface OperatorToolReceipt { tool:OperatorToolName; authority:Authority; request_digest:string; expected:Record<string,unknown>; observed:Record<string,unknown>; verified:boolean; rollback_ref:string|null; external_ref:string|null; detail:Record<string,unknown>; }

const TOOL_SPECS: Record<OperatorToolName,OperatorToolSpec> = {
 project_context_read:{name:"project_context_read",authority:0,mutating:false,reversible:true,description:"Read non-secret runtime/project configuration and tool availability."},
