import { db, isUndefinedTableError, newSchemaSlot, runOncePerDbClient } from "./db";
...
async function readOrEnsure<T>(query: () => Promise<T>): Promise<T> {
  try {
    return await query();
  } catch (error) {
...
export async function profile(userId: string) {
  const sql = db();
  return readOrEnsure(async () => {
    const r = await sql`select profile, updated_at from rmf_personal_profiles where user_id=${userId} limit 1`;
    return r[0] || null;
  });
}
...
export async function history(userId: string, limit=20) {
  const sql = db();
  return readOrEnsure(() =>
    sql`select id,kind,summary,data,created_at from rmf_interactions where user_id=${userId} order by created_at desc limit ${Math.min(Math.max(limit,1),50)}`
  );
}
...
export async function savedItems(userId:string,limit=20) {
  const sql = db();
  return readOrEnsure(() =>
    sql`select id,item_type,title,url,data,feedback,source_interaction_id,created_at,updated_at from rmf_personal_recommendations where user_id=${userId} order by created_at desc limit ${Math.min(Math.max(limit,1),50)}`
  );
}
