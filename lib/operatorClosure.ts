...
function managerialTasksetCandidate(signal:any):OperatorCandidate|null{
  const payload=record(signal?.payload);
  const action=record(payload.managerial_action);
  const kind=String(action.kind||payload.managerial_kind||"");
  const featureId=String(action.feature_id||payload.feature_id||"");
  const title=String(action.title||featureId);
  const acceptance=Array.isArray(action.acceptance)?action.acceptance.map(v=>String(v)).slice(0,12):[];
  if(kind==="verify"&&featureId){
    return{id:`verify-${featureId}`,tool:"feature_production_verify",authority:0,intent:`Verify ${title} against production health / feature flag / endpoint.`,reason:"Backlog advances only on a verified production receipt.",expected_return:"Production /api/health matches acceptance evidence.",reversible:true,invariants:["do_not_advance_without_verified_receipt","repo_flags_alone_do_not_ship","heartbeat_is_not_verification"],args:{feature_id:featureId}};
  }
  if(kind==="dispatch"&&featureId){
    return{id:`dispatch-${featureId}`,tool:"github_implementation_dispatch",authority:2,intent:`Dispatch an isolated-branch implementation path for ${title}.`,reason:"Unfinished Harry-specified feature — strategy reports are not done.",expected_return:"Isolated branch + dispatch artifact readback; optional draft PR; no merge.",reversible:true,invariants:["do_not_modify_base_branch","do_not_merge","independent_readback_must_match_expected_digest","halt_after_one_mutation","do_not_count_heartbeat_as_progress"],args:{feature_id:featureId,title,acceptance}};
  }
  if(kind==="blocked"&&String(action.blocked_on||"")==="approval"&&featureId){
    return{id:`dispatch-${featureId}`,tool:"github_implementation_dispatch",authority:2,intent:`Dispatch an isolated-branch implementation path for ${title}.`,reason:"L2 dispatch exceeds admitted authority — record approval, do not execute.",expected_return:"Isolated branch + dispatch artifact readback; optional draft PR; no merge.",reversible:true,invariants:["do_not_modify_base_branch","do_not_merge","independent_readback_must_match_expected_digest","halt_after_one_mutation","do_not_count_heartbeat_as_progress"],args:{feature_id:featureId,title,acceptance}};
  }
  return null;
}
function tasksetCandidate(signal:any):OperatorCandidate|null{
  const kind=String(signal?.kind||"");
  if(kind==="business_improve"||kind==="heartbeat")return managerialTasksetCandidate(signal);
  if(kind!=="control_probe")return null;
  const payload=record(signal?.payload);const requested=String(payload.probe||payload.capability||"");
  if(requested==="browser_observe"||requested==="browser"||payload.url){return{id:"control-probe-browser-observe",tool:"browser_observe",authority:2,intent:"Prove authenticated bounded browser observation through the Railway runtime without mutating the target account.",reason:"The browser L2 probe requires runtime health, two independent observations, matching snapshot digests, a receipt, and halt.",expected_return:"Authenticated Railway health plus two independently returned ChatGPT observation digests that agree.",reversible:true,invariants:["allowlisted_https_target_only","no_account_mutation","no_cookie_or_password_export","independent_reread_must_match","halt_after_receipt"],args:{url:String(payload.url||"https://chatgpt.com/")}};}
  return{id:"control-probe-github-branch",tool:"github_branch_diagnostic",authority:2,intent:"Demonstrate bounded external control by creating an isolated GitHub branch and one diagnostic receipt without merging or touching the base branch.",reason:"Deterministic GitHub control probe.",expected_return:"Verified isolated-branch readback receipt.",reversible:true,invariants:["do_not_modify_base_branch","do_not_merge","independent_readback_must_match_expected_digest","halt_after_one_mutation"],args:{}};
}
