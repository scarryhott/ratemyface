...
export async function GET(req:NextRequest){
 ...
 let data:any;
 if(mode==="history")data=await history(u.id,limit); else if(mode==="saved")data=await savedItems(u.id,limit); else if(mode==="connections")data=await connections(u.id); else if(mode==="report"){data={profile:await readProfileUnified(u.id),history:await history(u.id,20),saved:await savedItems(u.id,20),connections:await connections(u.id)};} else data=await readProfileUnified(u.id);
