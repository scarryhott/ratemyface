...
export async function POST(request: NextRequest) {
  ...
  const rows = await sql`
    select
      a.id as approval_id,a.run_id,a.capability,a.requested_authority,a.status as approval_status,a.rationale,
      r.signal_id,r.status as run_status,
      s.source,s.kind,s.payload,s.requested_authority as signal_requested_authority
  ...
  if (String(item.approval_status) !== "pending") {
    return NextResponse.json({ ok: false, error: "approval_already_decided", approval: item }, { status: 409 });
  }
  ...
  if (decision === "reject") {
    ...
    await sql`
      ...
      values(${item.run_id},'owner_rejected',${item.capability},${item.requested_authority},false,${sql.json({ approval_id: approvalId } as any)})
    `;
    return NextResponse.json({ ok: true, decision: "rejected", approval_id: approvalId });
  }
  ...
  const approvedPayload = {
    ...originalPayload,
    owner_approved: true,
    owner_approved_authority: Number(item.requested_authority),
    approval_id: approvalId,
    approved_from_run: Number(item.run_id)
  };
  ...
  const queued = await sql`
    insert into rmf_agent_signals(source,kind,payload,status,requested_authority)
    values('owner-approval',${item.kind},${sql.json(approvedPayload as any)},'queued',${item.requested_authority})
    returning id,status,requested_authority,created_at
  `;
  await sql`
    insert into rmf_agent_ledger(run_id,event,capability,authority,admissible,detail)
    values(${item.run_id},'owner_approved_requeued',${item.capability},${item.requested_authority},true,${sql.json({ approval_id: approvalId, new_signal_id: queued[0].id } as any)})
  `;
  ...
  return NextResponse.json({
    ...
    decision: "approved",
    approval_id: approvalId,
    requeued_signal: queued[0],
    hard_max_authority: Number(process.env.RMF_OPERATOR_MAX_AUTHORITY || 1),
    note: "Approval is recorded, but execution remains capped by RMF_OPERATOR_MAX_AUTHORITY."
  });
}
