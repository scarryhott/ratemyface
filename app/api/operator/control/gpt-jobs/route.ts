...
import { queueGptFactoryJob } from "../../../../../lib/unifiedControlPlane";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const job = await withDatabaseTimeout(
      () => queueGptFactoryJob(input as Record<string, unknown>, auth.actor || "operator"),
      OPERATOR_READ_TIMEOUT_MS
    );
    return NextResponse.json({ ok: true, job, actor: auth.actor }, { status: 202 });
  } catch (error) {
    ...
    const status = /_(invalid|required|forbidden)$/.test(message) ? 400 : 500;
