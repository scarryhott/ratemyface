...
export default function OperatorPage() {
  ...
  async function refresh() {
    await act(async () => {
      ...
      setCapabilities(caps); setStatus(current);
  ...
  async function enqueueProbe() {
    await act(() => request("/api/operator/signal", { method:"POST", body:JSON.stringify({ source:"owner-console", kind:"control_probe", requested_authority:2, payload:{ goal:"Demonstrate isolated GitHub branch control and independently verify the return." } }) }));
  }
  ...
  async function signOut() {
    ...
    setOwner(null); setCapabilities(null); setStatus(null);
  }
  ...
  return (
    <main>
      ...
      <div className="card">
        <h2 style={{marginTop:0}}>Owner authentication</h2>
        {owner ? (
          <>
            <p><strong>Signed in:</strong> {owner.email || owner.phone || owner.wallet || owner.id}</p>
          ...
          <>
            <p>Sign in with an allowlisted Google account, phone number, Ethereum wallet, or Solana wallet.</p>
            <a href="/operator/login" style={{display:"inline-block",marginTop:8}}>Open owner sign in</a>
            <hr style={{margin:"24px 0",border:0,borderTop:"1px solid #ddd"}}/>
            <label htmlFor="operator-secret">Machine/operator secret fallback</label>
            <input id="operator-secret" type="password" autoComplete="off" value={secret} onChange={(e)=>setSecret(e.target.value)} placeholder="RMF_OPERATOR_SIGNAL_SECRET" style={{width:"100%",marginTop:8,padding:12,border:"1px solid #bbb",borderRadius:8}} />
            <button onClick={refresh} disabled={loading || !secret} style={buttonStyle}>Inspect with secret</button>
          </>
        )}
      </div>

      {(capabilities || status) && <div className="card"><h2 style={{marginTop:0}}>Harness state</h2><p><strong>Harness:</strong> closure-native-v1</p><p><strong>Hard authority ceiling:</strong> L{String(hardMax)}</p><p><strong>AI Gateway:</strong> {gateway?"configured":"not configured"}</p><p><strong>GitHub L2 write:</strong> {githubWrite?"configured":"not configured"}</p><p><strong>Pending approvals:</strong> {pendingApprovals.length}</p><button onClick={refresh} disabled={loading} style={buttonStyle}>Refresh</button></div>}

      <div className="card"><h2 style={{marginTop:0}}>Signal the operator</h2><textarea value={message} onChange={(e)=>setMessage(e.target.value)} rows={5} style={{width:"100%",padding:12,border:"1px solid #bbb",borderRadius:8,resize:"vertical"}}/><div style={{display:"flex",gap:8,flexWrap:"wrap"}}><button onClick={enqueueAnalysis} disabled={loading||!authenticated} style={buttonStyle}>Queue L1 analysis</button><button onClick={runNext} disabled={loading||!authenticated} style={buttonStyle}>Run next signal</button></div><hr style={{margin:"24px 0",border:0,borderTop:"1px solid #ddd"}}/><p><strong>L2 control probe:</strong> isolated branch → deterministic artifact → independent readback → receipt → halt without merge.</p><button onClick={enqueueProbe} disabled={loading||!authenticated} style={buttonStyle}>Queue L2 control probe</button></div>
