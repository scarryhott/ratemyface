...
export default function OperatorPage() {
  ...
  return (
    <main>
      ...
      <p>Private control console for the closure-native agent that builds and operates GPT products. Ops overview: <a href="/operator/dashboard">/operator/dashboard</a>.</p>
      <div className="card"><h2 style={{marginTop:0}}>Owner authentication</h2>{owner ? <><p><strong>Signed in:</strong> {owner.email || owner.phone || owner.wallet || owner.id}</p><p><strong>Method:</strong> {owner.method}</p><div style={{display:"flex",gap:8,flexWrap:"wrap"}}><button onClick={refresh} disabled={loading} style={buttonStyle}>Inspect agent</button><a href="/operator/dashboard" style={buttonLinkStyle}>Ops overview</a><a href="/operator/access" style={buttonLinkStyle}>Account sign-in access</a><a href="/operator/browser" style={buttonLinkStyle}>Persistent browser login</a><button onClick={signOut} disabled={loading} style={secondaryButtonStyle}>Sign out</button></div></> : <><p>Sign in with an allowlisted Google account, phone number, Ethereum wallet, or Solana wallet.</p><a href="/operator/login" style={{display:"inline-block",marginTop:8}}>Open owner sign in</a><hr style={{margin:"24px 0",border:0,borderTop:"1px solid #ddd"}}/><label htmlFor="operator-secret">Machine/operator secret fallback</label><input id="operator-secret" type="password" autoComplete="off" value={secret} onChange={(e)=>setSecret(e.target.value)} placeholder="RMF_OPERATOR_SIGNAL_SECRET" style={{width:"100%",marginTop:8,padding:12,border:"1px solid #bbb",borderRadius:8}} /><button onClick={refresh} disabled={loading || !secret} style={buttonStyle}>Inspect with secret</button></>}</div>
